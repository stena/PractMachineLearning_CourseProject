...
The data was accessed on: 2014-06-22 04:05:00.
...
download.file("http://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv?accessType=DOWNLOAD", 
    csvTrainFile)
download.file("http://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv?accessType=DOWNLOAD", 
    csvTestFile)
...
```r
## Remove the column variable X from the data set because column variable X
## is equivalent to the rownames of the data set
...
factorVars <- which(colnames(trainData) %in% c("user_name", "raw_timestamp_part_1", 
    "raw_timestamp_part_2", "cvtd_timestamp"))

# convert non considered factor variables into numeric variables of training
# set
trainData[, -c(factorVars, which(colnames(trainData) == "classe"))] <- lapply(trainData[, 
    -c(factorVars, which(colnames(trainData) == "classe"))], as.numeric)
...
## Remove columns with more than 97% percent of values equal to NA (missing
## value)
...
## Remove columns with near zero variable, meaning variables with little
## variance Record variables that will be ignored for applying near zero
## variable (Conflicting data types (characters and dates), outcome variable)
ignoreVars <- which(colnames(tidyTrainData) %in% c("user_name", "cvtd_timestamp", 
    "classe"))

nsv <- nearZeroVar(tidyTrainData[, -ignoreVars], saveMetrics = TRUE)  # near zero variable columns from tidy training set
tidyTrainData <- tidyTrainData[, -which(colnames(tidyTrainData) %in% rownames(nsv[which(nsv$nzv == 
    TRUE), ]))]  # drop columns with near zero variable from tidy training set
tidyTestData <- tidyTestData[, -which(colnames(tidyTestData) %in% rownames(nsv[which(nsv$nzv == 
    TRUE), ]))]  # drop the same columns of tidy training set from tidy test set
```
...
inTrain <- createDataPartition(tidyTrainData$classe, p = 0.6, list = FALSE)  # partition data into a training (60%) and validation set (40%)
...
modelFit <- randomForest(classe ~ ., proximity = TRUE, data = training[, -which(colnames(tidyTrainData) %in% 
    c("user_name", "cvtd_timestamp"))])
```
...
## cross-tabulation of observed and predicted classes with associated
## statistics
...
## Sensitivity             1.000    0.999    0.996    0.995    0.999
...
The expected out of sample error is to be considered minimized, meaning that the accuracy of prediction is maximized. We expect this because the tree is grown to maximum size, without pruning. This subspace randomization scheme, is blended with bagging to resample with replacement of the training data set each time a new individual tree is grown. The accuracy has the value of 0.9981, meaning 99.8088%. This outcome is exactly what we expected, a high accuracy.
...
predictTest <- predict(modelFit, tidyTestData[, -which(colnames(tidyTestData) %in% 
    c("user_name", "cvtd_timestamp"))])
